...
try {
    ...
    $session->require_role(['inspectofficer', 'admin', 'signer', 'headquarter']);

    if (!$session->is_logged_in()) {
        throw new Exception('กรุณาเข้าสู่ระบบก่อน');
    }

    // 2) ดึงข้อมูล officer ปัจจุบัน
    ...
    if (!$officer->verify_password($current_password)) {
        throw new Exception('รหัสผ่านปัจจุบันไม่ถูกต้อง');
    }

    // 7) เช็คว่าอีเมลใหม่ซ้ำกับของคนอื่นหรือไม่
    if ($new_email !== $officer->email && Officer::exists_email($new_email)) {
        throw new Exception('อีเมลนี้ถูกใช้งานแล้ว');
    }

    // 8) เช็คว่าไม่ใช่อีเมลเดิม
    if (strcasecmp($new_email, $officer->email) === 0) {
