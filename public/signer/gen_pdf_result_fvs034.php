...
$source = '../../private/pdftemplate/FVS034.pdf';
...
// สร้าง URL สำหรับ QR
$url = 'https://fvscis.fisheries.go.th/verify_fvs034.php?token=' . urlencode($token);
...
$pdf->SetXY(60, 46);
$pdf->Cell(80, 7, iconv('UTF-8','cp874', (string)($form->document_number ?? '')));
...
$moo = !empty($applicant->applicant_moo)
    ...
    : '';
$address1 = "เลขที่ ".$applicant->applicant_address_no. $moo;
$address2 = " ตำบล ".$applicant->applicant_tambon." อำเภอ ".$applicant->applicant_amphoe . " จังหวัด ".$applicant->applicant_province ;

$pdf->SetXY(232, 114);
$pdf->Cell(80, 8, iconv('UTF-8','cp874', (string)($address1 ?? '')));

$pdf->SetXY(32, 122);
$pdf->Cell(80, 8, iconv('UTF-8','cp874', (string)($address2 ?? '')));
