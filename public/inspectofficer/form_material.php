...
// ====== โหลดหัวข้อหลักจาก inspection_main_items (section 2) ======
$section     = 2; // หมวดวัสดุ อุปกรณ์ และเครื่องมือ
$main_items  = InspectionMainItem::find_by_section_and_category($section, $form_type);

$main_item_ids      = [];
$id_to_section_code = []; // map main_item_id → "2_1", "2_2", ...

if (!empty($main_items)) {
    foreach ($main_items as $mi) {
        $code = trim((string)$mi->section_code); // เช่น "2_1"
        if ($code === '') { continue; }
...
$grouped_fail_items = []; // key = section_code ("2_1") => array ของ fail items
...
if (!empty($main_item_ids)) {
    ...
    if (!empty($fail_items)) {
        foreach ($fail_items as $fi) {
            ...
            $code = $id_to_section_code[$mid]; // "2_1", "2_2", ...
  ...
  <div class="accordion" id="inspectionAccordion">
    ...
    <?php if (!empty($main_items)): ?>
      <?php foreach ($main_items as $mi): ?>
        <?php
          $code   = $mi->section_code;              // เช่น "2_1"
          $title  = $mi->title_th;                  // ข้อความหัวข้อจาก DB

          $status_field = 'status_' . $code;        // status_2_1
          $remark_field = 'remark_' . $code;        // remark_2_1
          ...
          <div id="collapse<?= htmlspecialchars($code, ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div class="accordion-body">
              ...
              <form id="form-<?= htmlspecialchars($code, ENT_QUOTES, 'UTF-8') ?>"
                ...
                <!-- กล่องเหตุผลไม่ผ่าน (checkbox ตาม fail_items) -->
                  ...
                  <?php if ($fail_count > 0): ?>
                    <?php foreach ($fail_list as $fail_item): ?>
                      <?php
                        ...
                        if (strpos($raw_code, 'fail_') === 0) {
                            // ถ้าใน DB เก็บเป็น full field name เช่น "fail_2_1_1"
                            $fail_field = $raw_code;
                        } else {
                            // ถ้าใน DB เก็บเป็นเลขลำดับ เช่น "1", "2"
                            $fail_field = $basePrefix . $raw_code;
                        }
    ...
    <?php else: ?>
      <div class="alert alert-warning">
        ยังไม่มีหัวข้อการตรวจประเมินสำหรับหมวดวัสดุ อุปกรณ์ และเครื่องมือ (section 2)
      </div>
    <?php endif; ?>
