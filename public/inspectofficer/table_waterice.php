...
// section 4 = ด้านน้ำและน้ำแข็ง
$section = 4;

// ===== 1) ดึง main items ของหมวด 4 จาก inspection_main_items =====
...
if (!empty($main_item_ids)) {
    ...
    if (!empty($fail_items)) {
        foreach ($fail_items as $fi) {
            ...
            $code = $id_to_section_code[$mid]; // "4_1", "4_2", ...
          ...
          if ($status_val === 'fail') {
              ...
              if (!empty($fails_for_code)) {
                  foreach ($fails_for_code as $fi) {
                      // field name: fail_4_1_1, fail_4_1_2 ...
